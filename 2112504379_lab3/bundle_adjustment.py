...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    """
    obs_extrinsics = extrinsics[camera_idxs]
    obs_points3d = points3d[points3d_idxs]
    obs_points3d_homo = np.concatenate([obs_points3d, np.ones((obs_points3d.shape[0], 1))], axis=1)
    
    # Project points: P_cam = Extrinsics * P_world
    obs_points_cam = np.matmul(obs_extrinsics, obs_points3d_homo.reshape(-1, 4, 1)).reshape(-1, 3)
    
    # Apply intrinsics: P_img = Intrinsics * P_cam
    obs_points_proj_homo = np.matmul(intrinsics, obs_points_cam.reshape(-1, 3, 1)).reshape(-1, 3)
    
    # Normalize: x = X / Z, y = Y / Z
    obs_points_proj = obs_points_proj_homo[:, :2] / obs_points_proj_homo[:, 2:3]
    
    # Compute residuals (Euclidean distance)
    residuals = np.sqrt(np.sum(np.square(points2d - obs_points_proj), axis=1))
