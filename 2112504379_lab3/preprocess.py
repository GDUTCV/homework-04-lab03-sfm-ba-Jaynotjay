...
def create_ransac_matches(image_file1: os.path, image_file2: os.path,
    ...
    """ 
    YOUR CODE HERE:
    Use RANSAC to estimate an essential matrix and get inlier mask.
    Save the essential matrix and geometrically verified matches.
    """
    is_inlier = np.ones(shape=points1.shape[0], dtype=bool)  # dummy value
    essential_mtx = np.zeros(shape=[3, 3], dtype=float)

    points1_f = points1.astype(np.float32)
    points2_f = points2.astype(np.float32)

    if hasattr(cv2, 'findEssentialMat'):
        ...
        # Fallback for older OpenCV builds: estimate fundamental matrix then convert to essential.
        essential_mtx = None
        mask = None
        if hasattr(cv2, 'findFundamentalMat'):
            ...
            if F is not None and F.shape == (3, 3):
                essential_mtx = camera_intrinsics.T @ F @ camera_intrinsics

    if essential_mtx is None:
        essential_mtx = np.zeros(shape=[3, 3], dtype=float)
        is_inlier = np.zeros(shape=points1.shape[0], dtype=bool)
    else:
        if essential_mtx.shape != (3, 3):
            essential_mtx = essential_mtx.reshape(-1, 3, 3)[0]
        if mask is None:
            is_inlier = np.zeros(shape=points1.shape[0], dtype=bool)
        else:
            is_inlier = mask.reshape(-1).astype(bool)

    """ END YOUR CODE HERE """

    inlier_mask = is_inlier.reshape(-1).astype(bool)
    inlier_match_idxs = np.argwhere(inlier_mask).reshape(-1)
    if len(inlier_match_idxs) == 0:
        return match_id

    inlier_matches = match_idxs[inlier_match_idxs, :]
    np.save(match_save_file, inlier_matches)
    ...
    match_pts = np.concatenate([points1[inlier_match_idxs], points2[inlier_match_idxs]], axis=1)
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    """ 
    YOUR CODE HERE:
    Build a scene graph where an edge exists if the RANSAC-verified matches
    between the image pair has at least <min_num_inliers> correspondences.
    """
