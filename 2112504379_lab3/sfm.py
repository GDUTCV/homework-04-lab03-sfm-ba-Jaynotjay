...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    for _ in range(num_ransac_iterations):
        ...
        residuals = np.zeros(shape=num_pts, dtype=float)
        """ 
        YOUR CODE HERE 
...
def get_next_pair(scene_graph: dict, registered_ids: list):
    ...
    for registered_id in registered_ids:
        if registered_id in scene_graph:
            for neighbor_id in scene_graph[registered_id]:
                if neighbor_id not in registered_ids:
                    ...
                    if num_inliers > max_num_inliers:
                        ...
                        max_registered_id = registered_id    of image_id1. scene graph is modelled like an adjacency list.
    ...
    """ YOUR CODE HERE """
    for registered_id in registered_ids:
        if registered_id not in scene_graph:
            continue
        for neighbor_id in scene_graph[registered_id]:
            if neighbor_id in registered_ids:
                continue
            matches = load_matches(image_id1=neighbor_id, image_id2=registered_id)
            num_inliers = matches.shape[0]
            if num_inliers > max_num_inliers:
                max_num_inliers = num_inliers
                max_new_id = neighbor_id
